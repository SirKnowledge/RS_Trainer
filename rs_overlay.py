...
config_file = r"C:\Users\MartinB\OneDrive\Scripts\WorkForge\RS_Trainer\boss_rotations\azulyn_raksha_necro.json"
...
def play_game():
    ...
    if (str(config["see_ability_icons"])) == "True" or (
        str(config["see_ability_icons"])
    ) == "true":
        see_abilities = True
    else:
        see_abilities = False

    if (str(config["see_keybinds"])) == "True" or (
        str(config["see_keybinds"])
    ) == "true":
        see_keybinds = True
    else:
        see_keybinds = False
        ...
        def on_key_event(e):
            ...
            if e.event_type == "down":
                ...
                if key not in held_keys:
                    ...
                    key_press_count += 1
                    print(
                        f"[GLOBAL] Key pressed once: {key} (Total: {key_press_count})"
                    )
    ...
    while running:
        ...
        # Update animation
        if (str(config["see_global_cooldown_animation"])) == "True" or (
            str(config["see_global_cooldown_animation"])
        ) == "true":
        ...
        if key_down or mouse_clicked or new_global_key_events:
            ...
            if (
                str(config["default_exit_button"])
            ).lower() in new_global_key_events or (
                str(config["default_exit_button"])
            ).upper() in new_global_key_events:
                running = False
            ...
            for ability in spawned_abilities[:]:
                try:
                    ...
                    for k in ability.key:
                        ...
                        required_keys_pressed = all(
                            ...
                            if k == "ALT"
                                ...
                                if k == "MOUSE"
                                else keys[getattr(pygame, f"K_F{k[1:]}", None)]
                                if k.startswith("F") and k[1:].isdigit()
                                else keys[getattr(pygame, f"K_{k.lower()}", None)]
                    ...
                    if required_keys_pressed == False:
                        for k in ability.key:
                            ...
                            if k.lower() == "mouse":
                                ...
                                if (
                                    str(config["mouse_abilities"])
                                ) in new_global_key_events:
                            ...
                            # Check for modifier combinations
                                ...
                                "shift" in new_global_key_events
                                and f"shift+{k.lower()}" in key_combination_map
                            ):
                                if (
                                    key_combination_map[f"shift+{k.lower()}"]
                                    in new_global_key_events
                                ):
                                    print(
                                        f"Key {key_combination_map[f'shift+{k.lower()}']} is pressed with SHIFT"
                                    )
                                    required_keys_pressed = True

                            elif (
                                "ctrl" in new_global_key_events and k.lower() == "lctrl"
                            ):
                                print(f"Key is pressed with CTRL")
                                required_keys_pressed = True

                            elif (
                                "alt" in new_global_key_events
                                and f"alt+{k.lower()}" in key_combination_map
                            ):
                                if (
                                    key_combination_map[f"alt+{k.lower()}"]
                                    in new_global_key_events
                                ):
                                    print(
                                        f"Key {key_combination_map[f'alt+{k.lower()}']} is pressed with ALT"
                                    )
                                    required_keys_pressed = True
                    ...
                    if required_keys_pressed and press_zone_rect.colliderect(
                        ability.rect
                    ):
                    ...
                    print(
                        f"Warning: Unrecognized ability in ability: {ability.ability}"
                    )

            # Check for missed abilities after the loop
            if not ONE_ABILITY_HIT and (
                key_pressed or mouse_clicked or new_global_key_events
            ):
                if (
                    new_global_key_events == ["shift"]
                    or new_global_key_events == ["ctrl"]
                    or new_global_key_events == ["alt"]
                ):
                    pass
                else:
                    # print(key_pressed)
                    ...
                    if failed_attempts >= 3:
                        ...
                        for ability in spawned_abilities:
                            ability.visible = (
                                True  # Show the ability after 3 failed attempts
                            )
        # print("ahhhhhhhhhhhhhh" + str(new_global_key_events))
        new_global_key_events = [
            key
            for key in new_global_key_events
            if key.lower() == "shift" or key.lower() == "ctrl" or key.lower() == "alt"
        ]
        ...
        valid_spawned_abilities = [
            ability for ability in spawned_abilities if not ability.image == None
        ]
        if current_time >= next_tick_time and not valid_spawned_abilities:
            ...
            future_ticks = [
                ability["tick"]
                for ability in ability_sequence
                if ability["tick"] > current_tick
            ]
            ...
            for ability_data in ability_sequence:
                ...
                if ability_data["tick"] == current_tick:
                    ...
                    if ability in ABILITY_KEYBINDS and ability in ABILITY_IMAGES:
                        ...
                        width = ability_data.get(
                            "width", ABILITY_DEFAULT_WIDTH
                        )  # Default width to 75 if not provided
                        # Debug log for missing abilities

                        print(
                            f"Spawning ability: {ability}, Keys: {key}, Tick: {current_tick}, Width: {width}, tick_count: {tick_count}, ABILITY_SPACING_Y: {ABILITY_SPACING_Y}"
                        )
                        ...
                        print(
                            f"Warning: Ability {ability} not found in keybinds or images."
                        )
                        if "Text:" in ability:
                            ...
                            ability = Ability(
                                ...
                                image_path="None",
                                start_x=press_zone_rect.x
                                - 10,  # Place ability on press_zone_rect
                                start_y=ability_y,
                                width=width / 2,
                                stationary=True,  # Mark ability as stationary
                                visible=see_abilities,
                ...
                if ability_data["tick"] == next_ability:
                    ...
                    if ability in ABILITY_KEYBINDS and ability in ABILITY_IMAGES:
                        ...
                        ability_y = (win_h // 4) + (
                            tick_count_queue * (ABILITY_SPACING_Y)
                        )
                        if key == []:
                            key = ["MOUSE"]
                        ability = Ability(
                            ...
                            image_path=image_path,
                            start_x=press_zone_rect.x
                            + ABILITY_SPACING_X,  # Place ability on press_zone_rect
                            start_y=ability_y,
                            width=width / 2,
                            stationary=True,  # Mark ability as stationary
                            visible=see_abilities,
                        ...
                        if "Text:" in ability:
                            ...
                            spawned_abilities_queue.append(ability)
                            tick_ability_counts_queue[next_ability] = (
                                tick_count_queue + 0
                            )
                ...
                if ability_data["tick"] == next_next_ability:
                    ...
                    if ability in ABILITY_KEYBINDS and ability in ABILITY_IMAGES:
                        ...
                        ability_y = (win_h // 4) + (
                            tick_count_queue2 * (ABILITY_SPACING_Y)
                        )
                        if key == []:
                            key = ["MOUSE"]
                        ability = Ability(
                            ...
                            image_path=image_path,
                            start_x=press_zone_rect.x
                            + ABILITY_SPACING_X * 2,  # Place ability on press_zone_rect
                            start_y=ability_y,
                            width=width / 2,
                            stationary=True,  # Mark ability as stationary
                            visible=see_abilities,
                        ...
                        spawned_abilities_queue.append(ability)
                        tick_ability_counts_queue2[next_next_ability] = (
                            tick_count_queue2 + 1
                        )
                    else:
                        print(
        ...
        if feedback_message and time.time() < feedback_timer:
            ...
            if (str(config["see_correct_or_incorrect_feedback_message"])) == "True" or (
                str(config["see_correct_or_incorrect_feedback_message"])
            ) == "true":
...
def prompt_restart():
    ...
    waiting = True
    while waiting:
        if (str(config["default_restart_button"])).lower() in new_global_key_events or (
            str(config["default_restart_button"])
        ).upper() in new_global_key_events:
            new_global_key_events.clear()
            still_active_global_key_events.clear()
            return True
        if (str(config["default_exit_button"])).lower() in new_global_key_events or (
            str(config["default_exit_button"])
        ).upper() in new_global_key_events:
            new_global_key_events.clear()
            return False
...
def trigger():
    while True:
        ...
        if not prompt_restart():
            break
    keyboard.unhook(remove=testvariablehook)
    pygame.quit()
